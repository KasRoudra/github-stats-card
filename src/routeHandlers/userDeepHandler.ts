...
import userDeepCard from "../cards/userDeepCard";
import errorCard from "../cards/errorCard";
import themes from "../themes";
import { userDeepResponse } from "../interfaces";
import {
  parseString,
  parseNumber,
  parseColor,
...
const userDeepHandler = async (req: Request, res: Response) => {
  try {
    ...
    const width =
      (req.query.width as string) || (layout === "compact" ? "470" : "360");
    const height =
      (req.query.height as string) || (layout === "compact" ? "310" : "350");
    ...
    if (username && token) {
      fetch("https://api.github.com/graphql", {
        ...
        .then((resp) => resp.json())
        .then(async (response: userDeepResponse) => {
          if (response.data) {
            ...
            if (user) {
              ...
              let starCount: number | string = 0,
                ...
                commitCount: number | string = 0,
                changeCount: number | string = 0;
              for (let i = 0; i < repoLength; i++) {
                ...
                if (repos[i].defaultBranchRef) {
                  commitCount +=
                    repos[i].defaultBranchRef.target.history.totalCount;
                  const edges = repos[i].defaultBranchRef.target.history.edges;
                  let oneChangeCount = 0;
                  for (let j = 0; j < edges.length; j++) {
                    oneChangeCount =
                      oneChangeCount +
                      edges[j].node.additions +
                      edges[j].node.deletions;
                  }
                  changeCount += oneChangeCount;
                }
                if (repos[i].languages) {
                  const langLength = repos[i].languages.nodes.length;
                  for (let j = 0; j < langLength; j++) {
                    langs.push(repos[i].languages.nodes[j].name);
                  }
                }
              }
              const pulls = user.pullRequests.nodes;
              if (pullCount) {
                for (let i = 0; i < pulls.length; i++) {
                  changeCount =
                    changeCount + pulls[i].additions + pulls[i].deletions;
                }
              }
              const langCount = Array.from(new Set(langs)).length;
              starCount = parseNumber(starCount);
              forkCount = parseNumber(forkCount);
              watcherCount = parseNumber(watcherCount);
              commitCount = parseNumber(commitCount);
...
export default userDeepHandler;
